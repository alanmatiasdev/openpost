import { VIDEO_PROJECT_SCHEMA_VERSION, type VideoProjectDocumentV1 } from "./types.js";
...
export function migrateVideoProjectDocument(input: unknown): VideoProjectMigrationResult {
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    throw new Error("OpenPost Video Editor project data must be a JSON object.");
  }
  const sourceVersion = Number((input as Record<string, unknown>).schema_version);
