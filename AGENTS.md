...
- Run the closest root gate first with a surface scope or policy selector: `bun run check -- <scope>`. Use `bun run doctor` before broad, browser, or release work; `bun run release -- check` before releases; `bun run verify` only for high-risk local build proof.
- UI and copy changes must pass the `ux-consistency` acceptance bar before finishing.
- Behavior changes: sync docs, product copy, contracts, and `CHANGELOG.md` under `Unreleased`.
