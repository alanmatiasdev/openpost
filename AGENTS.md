...
- **Push-to-deploy:** Production releases are tag-driven. `bun run release:prod "<commit>"` derives SemVer from Conventional Commits, runs the exact local release gate, pushes `main`, waits for the full CI matrix on that SHA, and only then creates the tag. CI builds and restart-smokes `ghcr.io/rodrgds/openpost:sha-<revision>` once. `.github/workflows/release.yml` promotes that tested image without rebuilding it, publishes the GitHub release, and calls the signed VPS hook with the revision, tag, and digest. The NixOS deploy script validates the production config, deploys the immutable digest, verifies `/api/v1/version`, and automatically restores the prior image if readiness fails.
...
- **Production release flow:** start with `bun run release:plan` and the fast `bun run release:preflight`, then run `bun run release:prod "<conventional commit message>"`. Preflight checks disk, Docker, GitHub access, workflows, the deploy secret, and public readiness before long work; if host space is below the release threshold while Docker still responds, it first prunes only unused BuildKit cache. The full `release:check` runs the canonical gates, race and security checks, isolated browser suites, and a restart-smoked production image, then reuses an exact-worktree fingerprint for 24 hours; set `OPENPOST_FORCE_RELEASE_CHECK=1` to force a rerun. Use `release:prepare` to stop after the exact SHA passes local checks and hosted CI, and `release:promote <tag>` to tag and deploy that prepared SHA. `RELEASE_BUMP` may only raise inferred impact; `RELEASE_VERSION` is reserved for an intentional greater version-line correction and cannot undercut commit impact. If a tag fails, do not retag it; fix forward with a new SemVer tag.
