...
- `GET /api/v1/workspaces/{id}/setup`: projects the current user's setup guidance from the Workspace name, hosted subscription, active destinations, and scheduled or submitted Publications. It stores no separate onboarding state. The home, Accounts, and composer surfaces re-read this projection after refresh and return flows; viewers receive no setup action.
