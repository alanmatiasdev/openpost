...
for (const [slug, comparison] of Object.entries(comparisonEvidenceRegister)) {
	...
	assert.ok(comparison.qualifier.length >= 40, `${slug} needs a useful plan/region qualifier`);
	assert.equal(
	...
	for (const [area, claim] of Object.entries(comparison.rows)) {
		assert.ok(['Direct source', 'Interpretation'].includes(claim.basis), `${slug}/${area} basis`);
		assert.ok(claim.qualifier.length >= 30, `${slug}/${area} needs a useful qualifier`);
		assert.ok(claim.sources.length > 0, `${slug}/${area} needs a source`);
		const sourceURLs = new Set();
		for (const source of claim.sources) {
			const url = new URL(source.href);
			assert.equal(url.protocol, 'https:', `${slug}/${area} source must use HTTPS`);
			assert.ok(source.label.length > 3, `${slug}/${area} source label is too short`);
			assert.equal(sourceURLs.has(source.href), false, `${slug}/${area} duplicates ${source.href}`);
