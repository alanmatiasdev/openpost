...
export function validateMarketingClaims(manifest, sources, now = new Date()) {
  ...
  assert.ok(nextReview >= startOfDay(now), "marketing claim register review is overdue");
  ...
  for (const [index, claim] of manifest.claims.entries()) {
    ...
    assert.equal(typeof claim.exact_text, "string", `${label}.exact_text is required`);
    assert.equal(typeof claim.evidence, "string", `${label}.evidence is required`);
  ...
  for (const phrase of ["Used by builders at", "Trusted by", "customers use OpenPost"]) {
    if (marketingSource.includes(phrase)) {
      assert.ok(registeredText.has(phrase), `unregistered public proof claim: ${phrase}`);
    }
  }
  for (const match of marketingSource.matchAll(/\/assets\/customer-logos\/[A-Za-z0-9._/-]+/g)) {
    assert.ok(registeredAssets.has(match[0]), `unregistered customer logo: ${match[0]}`);
  }
  ...
  for (const [index, material] of manifest.illustrative_material.entries()) {
    ...
    assert.equal(typeof material.source, "string", `${label}.source is required`);
    assert.equal(typeof material.label, "string", `${label}.label is required`);
    assert.equal(typeof material.description, "string", `${label}.description is required`);
    const source = sources.get(material.source);
    assert.ok(source, `${label}.source does not exist`);
    assert.ok(source.includes(material.label), `${label}.label is not visible in its source`);
    assert.match(source, /fictional/i, `${label}.source must state that the examples are fictional`);
  }
  ...
  for (const [sourcePath] of generatedPortraitSources) {
    assert.ok(
      manifest.illustrative_material.some((material) => material.source === sourcePath),
...
async function main() {
  ...
  console.log("Marketing claim register is current and public proof claims are registered.");
}
