...
export const REQUIRED_ARTIFACTS = [
  ...
  ...["grilling", "to-spec", "to-tickets", "implement", "tdd", "code-review", "triage", "handoff", "codebase-design"].map(
...
export function parseLiveLabels(json) {
  ...
  return new Set(value.map((label) => (typeof label === "string" ? label : label?.name)).filter(Boolean));
}
...
export async function main({ root = process.cwd(), output = console } = {}) {
  ...
  if (auth.error?.code === "ENOENT" || auth.status !== 0) {
    output.warn("Agent doctor: live label validation skipped (gh is unavailable or not authenticated).");
    return 0;
  }
  ...
  try {
    ...
    output.warn(`Agent doctor: live label validation skipped (invalid gh response: ${error.message}).`);
    return 0;
  }
  ...
  output.error("Review and run the commands above to create labels; this doctor never changes GitHub state.");
  return 1;
}
