...
8. Deliver: commit directly to `main` with Conventional Commits as work progresses; push at the end and close the completed issues only when asked. A push is not a deployment. Use `handoff` only when another context or human must continue unfinished work; otherwise close with verified results and remaining tickets.
