...
- Kept the exact Markdown `Accept` gate within the function set Cloudflare permits in every deployed edge phase, so the reviewed plan can be applied without weakening rejection of mixed, weighted, wildcard, parameterized, repeated, or whitespace-padded values.
