			...
			<article class="rounded border border-[oklch(0.26_0.012_55)] bg-[oklch(0.18_0.008_55)] p-1.5">
				<div class="mb-1 flex items-center justify-between gap-1">
					<span class="text-[10px] text-[oklch(0.76_0.012_55)]">
						{m.video_editor_local_ai_duration({ seconds: generation.result.duration.toFixed(1) })}
					</span>
					<button
						type="button"
				...
				<div class="mt-1 grid grid-cols-2 gap-1">
					...
					>
						{generation.mediaId ? m.video_editor_local_ai_saved() : m.video_editor_local_ai_save()}
					</Button>
					<Button
						size="xs"
						variant="secondary"
