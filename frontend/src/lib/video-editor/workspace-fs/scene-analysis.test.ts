...
beforeEach(() => {
	...
	vi.spyOn(fs, 'readArrayBuffer').mockImplementation(
		async (_root: FileSystemDirectoryHandle, path: string[]) =>
			storage.buffers.get(path.join('/')) ?? null
	);
	...
	vi.spyOn(fs, 'writeJsonAtomic').mockImplementation(
		storage.writeJsonAtomic as typeof fs.writeJsonAtomic
	);
