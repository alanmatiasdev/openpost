<script lang="ts">
	...
	const targetAccounts = $derived(accounts.filter((account) => account.supports_repost && !account.grant_required));
	const summary = $derived.by(() => {
		if (value.mode === 'off') return m.composer_repost_off();
		if (value.mode === 'custom') {
			const count = value.target_account_ids?.length ?? 0;
			return count === 1
				? m.composer_repost_one_account()
				: m.composer_repost_accounts({ count });
		}
	...
	function setDelay(seconds: number) {
		...
		if (value.rule.evaluation_window_seconds < seconds) {
			value.rule.evaluation_window_seconds = windowOptions.find((option) => option.value >= seconds)?.value ?? 2592000;
		}
		onChange?.();
	}

	function setThreshold(field: 'min_likes' | 'min_comments' | 'min_reposts' | 'min_views', raw: string) {
		if (value.mode !== 'custom' || !value.rule) return;
...
<Popover.Root bind:open>
	...
	<Popover.Content align="start" class="max-h-[min(72vh,42rem)] w-[min(92vw,30rem)] overflow-y-auto p-0">
		...
		>
			{#each [
				['inherit', m.composer_repost_default(), m.composer_repost_default_body()],
				['off', m.composer_repost_never(), m.composer_repost_never_body()],
				['custom', m.composer_repost_custom(), m.composer_repost_custom_body()]
			] as option (option[0])}
				<label class={[
			...
			<div class="space-y-5 border-t p-4">
				<fieldset class="space-y-2">
					...
					{:else}
						<div class="grid gap-2 sm:grid-cols-2">
							...
							{:else}
								<p class="text-sm text-muted-foreground sm:col-span-2">{m.repost_no_supported_accounts()}</p>
							{/each}
						</div>
					{/if}
				</fieldset>
				...
				<div class="grid gap-3 sm:grid-cols-2">
					<div class="space-y-2">
						...
						<Select.Root type="single" value={String(value.rule.delay_seconds)} onValueChange={(raw) => setDelay(Number(raw))}>
							<Select.Trigger class="w-full">{delayLabel(value.rule.delay_seconds, delayOptions)}</Select.Trigger>
							<Select.Content>
					...
					<div class="space-y-2">
						...
						<Select.Root type="single" value={String(value.rule.evaluation_window_seconds)} onValueChange={(raw) => updateRule((rule) => (rule.evaluation_window_seconds = Number(raw)))}>
							<Select.Trigger class="w-full">{delayLabel(value.rule.evaluation_window_seconds, windowOptions)}</Select.Trigger>
							<Select.Content>
								{#each windowOptions as option (option.value)}
									<Select.Item value={String(option.value)} disabled={option.value < value.rule.delay_seconds}>{option.label}</Select.Item>
								{/each}
							</Select.Content>
						</Select.Root>
					</div>
				...
				<div class="space-y-3 border-t pt-4">
					...
					<div class="grid grid-cols-2 gap-3">
						{#each [
							['min_likes', m.repost_min_likes()],
							['min_comments', m.repost_min_comments()],
							['min_reposts', m.repost_min_reposts()],
							['min_views', m.repost_min_views()]
						] as threshold (threshold[0])}
							<div class="space-y-1.5">
								...
								<Input
									...
									oninput={(event) => setThreshold(threshold[0] as 'min_likes' | 'min_comments' | 'min_reposts' | 'min_views', (event.target as HTMLInputElement).value)}
								/>
							</div>
						{/each}
					</div>
					<Select.Root type="single" value={value.rule.threshold_mode} onValueChange={(mode) => updateRule((rule) => (rule.threshold_mode = mode as 'all' | 'any'))}>
						<Select.Trigger class="w-full">{value.rule.threshold_mode === 'all' ? m.repost_require_all() : m.repost_require_any()}</Select.Trigger>
						<Select.Content>
					...
					{#if value.rule.require_plateau}
						<Select.Root type="single" value={String(value.rule.plateau_checks)} onValueChange={(raw) => updateRule((rule) => (rule.plateau_checks = Number(raw)))}>
							<Select.Trigger class="w-full">{m.repost_check_count({ count: value.rule.plateau_checks })}</Select.Trigger>
							<Select.Content>
