...
test("sidebar and full calendar use the same canonical publication inventory", async ({
  ...
  const auth = await registerUser(request, `calendar-inventory-${unique}@example.com`);
  const workspace = (await createWorkspace(request, auth.token, "Calendar inventory")) as {
    id: string;
    name: string;
  };
  ...
  const publications = [
    publication("published-one", workspace.id, "published", "2030-06-13T10:00:00Z"),
    publication("published-two", workspace.id, "published", "2030-06-14T11:00:00Z"),
    publication("scheduled-one", workspace.id, "scheduled", "2030-06-15T12:00:00Z"),
  ];
  ...
  await expect(
    page.getByTestId("sidebar-rolling-calendar").locator('span[aria-hidden="true"].rounded-full'),
  ).toHaveCount(3);
...
test("desktop month view keeps dense days compact and reveals the full day", async ({
  ...
  const unique = Date.now().toString(36);
  const auth = await registerUser(request, `calendar-density-${unique}@example.com`);
  const workspace = (await createWorkspace(request, auth.token, "Calendar density")) as {
    id: string;
    name: string;
  };
  await authenticatePage(page, auth.token);
  ...
  const publications = [
    ...
    publication("dense-three", workspace.id, "scheduled", "2030-06-15T15:00:00Z"),
    publication("dense-four", workspace.id, "scheduled", "2030-06-15T16:00:00Z"),
  ];
  await page.route("**/api/v1/publications?**", async (route) => {
    await route.fulfill({
      contentType: "application/json",
  ...
  expect((monthBounds?.y ?? 0) + (monthBounds?.height ?? 0)).toBeLessThanOrEqual(820);
