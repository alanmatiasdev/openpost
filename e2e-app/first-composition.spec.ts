...
test("meaningful composition starts once across focus, empty drafts, refresh, and composer instances", async ({
  ...
  const auth = await registerUser(
    request,
    `first-composition-${unique}@example.com`,
  );
  const workspace = (await createWorkspace(
    request,
    auth.token,
    `First composition ${unique}`,
  )) as { id: string };
  ...
  const setupResponse = await request.get(
    `/api/v1/workspaces/${workspace.id}/setup`,
...
test("media and content-mode input start composition without exposing content", async ({
  ...
  const auth = await registerUser(
    request,
    `composition-signals-${unique}@example.com`,
  );
  const workspace = (await createWorkspace(
    request,
    auth.token,
  ...
  const secondAuth = await registerUser(
    request,
    `composition-media-${unique}@example.com`,
  );
  ...
  page.on("request", (browserRequest) => {
    if (
      ...
      mediaSignals.push(
        (browserRequest.postDataJSON() as { signal: string }).signal,
      );
    }
  });
  await page.goto("/");
  ...
  await picker
    .getByRole("button", { name: "Select first-composition.png" })
    .click();
