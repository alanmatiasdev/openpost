...
test("one-hour 1080p60 source stays bounded and responsive", async ({ page }, testInfo) => {
  test.skip(!sourcePath, "Set OPENPOST_VIDEO_EDITOR_BENCHMARK_SOURCE to the long fixture.");
  ...
  for (const ratio of [0.03, 0.17, 0.34, 0.51, 0.68, 0.84, 0.97]) {
    ...
    await expect
      .poll(async () => Number(await preview.getAttribute("data-preview-rendered-timestamp-us")), {
        timeout: 5_000,
      })
  ...
  const droppedBefore = Number(await preview.getAttribute("data-preview-dropped-requests"));
  ...
  await expect
    .poll(async () => playbackVideo.evaluate((element: HTMLVideoElement) => element.currentTime), {
      timeout: 10_000,
    })
    .toBeGreaterThan(playbackStartSeconds + 0.1);
  const playback = await playbackVideo.evaluate(async (element: HTMLVideoElement) => {
    ...
    return {
      ...
      decoded_frames:
        (finalQuality?.totalVideoFrames ?? 0) - (initialQuality?.totalVideoFrames ?? 0),
      dropped_frames:
        (finalQuality?.droppedVideoFrames ?? 0) - (initialQuality?.droppedVideoFrames ?? 0),
    };
  });
  await page.getByRole("button", { name: "Pause" }).first().click();
  const droppedAfter = Number(await preview.getAttribute("data-preview-dropped-requests"));
  const sortedFrameIntervals = [...playback.frame_intervals_ms].sort((left, right) => left - right);
  ...
  const report = {
    ...
    seek_p95_ms: Math.round(sortedSeeks[Math.floor(sortedSeeks.length * 0.95)] ?? 0),
    ...
    peak_video_decoders: Number(await preview.getAttribute("data-preview-peak-decoders")),
    proxy_sources: Number(await preview.getAttribute("data-preview-proxy-sources")),
  };
